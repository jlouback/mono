...
public class CannyEdgeDetector {
	...
	//NEW-To compute edges
	private ArrayList<Point> borderCoordinates;
	...
	public CannyEdgeDetector() {
		...
		borderCoordinates = new ArrayList<Point>();
	}

	// accessors
	
	/**
	 * New - returns matrix of edge coordinates
	 * 
	 */
	public ArrayList<Point> getBorderCoordinates() {
		...
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += width) {
				...
				if (xGrad * yGrad <= (float) 0 /*(1)*/
					...
					//NEW add coordinates to edge detector
					if(gradMag >= highThreshold) {
						int xEdge = index % width;
						int yEdge = index/width;
						if(xEdge > 10 && xEdge < (width-10)) {
							borderCoordinates.add(new Point(xEdge, yEdge));
						}
					}
					//End of NEW
